...
export default function BottomNav({ user }: BottomNavProps) {
  ...
  const navigation = [
    ...
    { name: "Guests", href: "/dashboard/guests", icon: Users, roles: ["super_admin", "admin", "coordinator", "viewer"] },
    { name: "Settings", href: "/dashboard/settings", icon: Settings, roles: ["super_admin", "admin", "coordinator", "viewer"] },
  ];

  return (
    <nav className="fixed bottom-0 left-0 right-0 border-t border-zinc-900 bg-zinc-950 px-4 py-2 flex justify-around items-center md:hidden z-50 shadow-lg">
      {navigation.map((item) => {
